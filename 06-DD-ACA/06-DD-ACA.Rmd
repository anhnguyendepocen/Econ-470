...
![:col_row Treated, $E(Y_{1}(1)|W=1)$, $E(Y_{1}(0)|W=1)$]
...
<br>
Strategy 1: Estimate $E[Y_{0}(1)|W=1]$ using $E[Y_{1}(0)|W=1]$ (before treatment outcome used to estimate post-treatment)
...
# Estimation
Regression:<br>
$ Y_{i} = \alpha + \beta W_{i} + \lambda 1(Post) + \delta W_{i} 1(Post) + \varepsilon $
...
# Regression results
```{r}
ins.dat.2014 <- ins.dat %>% mutate(post = (year>=2014)) %>% filter(is.na(expand_year) | expand_year==2014)
