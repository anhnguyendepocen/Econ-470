...
The first line reads from left to right and from data to operation.

The `Base R` version (line 2) works in the opposite order.
...
Use `select(contains(PATTERN))` to find rows that contain some strings of interes
```{r select3}
starwars %>% 
...
2. `separate()`: Splie one column into multiple columns

3. `unite()`: Comine multiple columns into one
...
First step is to download the raw data that we'll be using. Let's focus on 2018 for now:
 ...
 - [Plan Characteristics](https://www.cms.gov/Medicare/Prescription-Drug-Coverage/PrescriptionDrugCovGenIn/)
...
monthlist=c("01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12")
  for (m in monthlist) {
    ...
    contract.info=read.csv(ma.path,
                           col.names=c("contractid","planid","org_type","plan_type","partd","snp",
                                       "eghp","org_name","org_marketing_name","plan_name","parent_org","contract_date"),
...
```{r eval=FALSE, include=T}
    ...
    enroll.info=read.csv(ma.path,
                         col.names=c("contractid","planid","ssa","fips","state","county","enrollment"),
                         stringsAsFactors=FALSE)
...
```{r eval=FALSE, include=T}
    ...
    plan.data.2018 = contract.info %>%
      left_join(enroll.info, by=c("contractid", "planid")) %>%
      mutate(month=as.numeric(m),year=2018)
}
```
...
```{r eval=FALSE, include=T}
  ...
  for (m in monthlist) {
    step=step+1
    assign(paste("plan.month.",step,sep=""),get(paste0("plan.data.2018.",m)))
    if (step==1) {
