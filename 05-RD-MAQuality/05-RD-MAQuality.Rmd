...
anim_save("rd_animate.gif",
          anim.p,path="C:/Users/immccar/CloudStation/Professional/Teaching Material/Emory/Econ 470 - Economics and Health Policy/05-RD-MAQuality/pics")
```

.center[
  ![:scale 800px](pics/rd_animate.gif)
]
...
3. Estimate regressions...
  ...
  - Nonlinear: add polynomials in $\tilde{X}$ and interactions $W \tilde{X}$ (i.e, $\tilde{X}^{2}$ and $\tilde{X}^{2} W$. 
...
rd.dat2 <- tibble(
  ...
  X = runif(n,0,2)+eps,
  W = (X>1),
  Y = 0.5 + 2*X + 4*W + eps + rnorm(n,0,.5) + 1.5*Z
)
...
# Full MA Data
```{r eval=T, include=T}
ma.data <- read_rds("C:/Users/immccar/CloudStation/Professional/Research Projects/_Git/Medicare-Advantage/data/ma_data.rds")
```
