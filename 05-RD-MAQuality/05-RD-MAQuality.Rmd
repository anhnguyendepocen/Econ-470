...
```{r}
library(rdrobust)
rd.result <- rdplot(rd.dat$Y, rd.dat$X, c=1, title="RD Plot with Binned Average", x.label="Running Variable", y.label="Outcome")
...
---
# Selecting "bin" width
1. Dummy variables: Create dummies for each bin, regress the outcome on the set of all dummies $R^{2}_{r}$, repeat with double the number of bins and find r-square value $R^{2}_{u}$, form F-stat, $\frac{R^{2}_{u] - R^{2}_{r}}{1-R^{2}_{u}}*\frac{n-K-1}{K}$.
...
---
# Fuzzy RD is IV
In practice, fuzzy RD is employed as an instrumental variables estimator, where we take the the difference in outcomes among those above and below the discontinuity divided by the difference in treatment probabilities for those above and below the discontinuity: $$
