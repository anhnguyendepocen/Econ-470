...
pacman::p_load(tidyverse, ggplot2, dplyr, lubridate, stringr, readxl, data.table, gdata, scales,
               stargazer, ivpack, kableExtra, broom, rdd, rdrobust, rddensity, cobalt)
...
ma.data <- readRDS(paste0(path.ma.data,'/ma_data.rds'))
...
## Summary, Question 1
snp.800.plans <- full.ma.data %>% 
  filter(snp == "Yes" | (planid >= 800 & planid < 900)) %>%  
  mutate(special_plan = case_when(
    snp=="Yes" ~ "SNP",
    planid>=800 & planid<900 ~ "800_series")
  )


snp.plot <- snp.800.plans %>% filter(special_plan=="SNP") %>%
  group_by(fips, year) %>% 
...
series800.plot <- snp.800.plans %>% filter(special_plan=="800_series") %>%
  group_by(fips, year) %>% 
  select(fips, year) %>% summarize(plan_count=n()) %>%
  ggplot(aes(x=as.factor(year),y=plan_count)) + 
  stat_summary(fun.y="mean", geom="bar") +
...
## Summary, Question 3
ratings.years <- ma.data %>% filter(year==2009 | year==2012 | year==2015, !is.na(Star_Rating)) %>%
...
## Summary, Question 4
avg.benchmark <- ma.data %>% filter(year>2007) %>% group_by(ssa, year) %>%
...
rm(list=c("full.ma.data", "ma.data","snp.800.plans","ma.mkt.data", "ma.data.clean"))
...
rmarkdown::render(input = 'hwk-04-answers.Rmd',
                  output_file ='hwk-04-answers')
