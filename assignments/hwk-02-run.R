...
hcris.data.v1996 <- readRDS(paste0(path.hcris.data,'/HCRIS_Data_v1996.rds'))
hcris.data.v2010 <- readRDS(paste0(path.hcris.data,'/HCRIS_Data_v2010.rds'))
hcris.data <- readRDS(paste0(path.hcris.data,'/HCRIS_Data.rds'))
...
fig.dup <- dup.count %>%
  ...
  labs(
    ...
    title="Hospitals with Duplicate Reports per Year"
  ) +
  theme_bw() +
...
fig.totcharge <- charge.data %>%
  ...
  labs(
    x="Year",
    y="Log $",
    title="Hospital Charges per Year"
  ) + 
  theme_minimal() +
  theme(
...
fig.pen <- pen.data %>% group_by(year) %>%
  ...
  labs(
    x="Year",
    y="Percent Penalized",
    title="Share of Hospitals Penalized per Year"
  ) + 
...
avg.pen <- pen.data %>% filter(year==2012) %>% 
  group_by(penalty) %>% summarize(mean_price=mean(price))

## Final data for matching
...
rmarkdown::render(input = 'hwk-02-answers.Rmd',
                  output_file ='hwk-02-answers')
